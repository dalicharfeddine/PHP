...
$formation=new favorieC();
//$prod=$formation->Afficherfavorieparutilisateur($_SESSION['id']);
/*
     ...
     if(!empty($_SESSION['rol']))
{
    ...
    if($_SESSION['rol']==1)
    {
   $pr=$formation->afficherfavorieseul($pro['id_formation'],$_SESSION['id']);
   foreach($pr as $fav){
    $x=$fav['id_favorie'];
   }
...
?>
 ...
 <form  method="post" action="supprimerfav.php">
                                
                           <input type="hidden" name="id_favorie"  value="<?php echo $x?>">
                           <button type="submit" name="Ajouter" value="" class="btn btn-danger">Supprimer</button>
